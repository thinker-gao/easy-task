...
<a href="./README_zh.md">English document</a> | <a href="./README_zh.md">Chinese document</a>
<p align="">
...
<p>&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp;EasyTask is a Composer package for PHP's resident memory timer / scheduled task. Through its simple API, you can use it to complete tasks that need to be run repeatedly (such as automatic cancellation of order timeout, asynchronous push of SMS mail, queue / consumer / channel subscription ), And can also complete some scheduled tasks (such as synchronizing DB data from 1 am to 3 am every day, generating monthly unified reports on the 1st of every month, and restarting the nginx server at 10 pm every night); at the same time, we also integrated the task abnormal reporting function, You can customize the handling of abnormal errors (such as automatic SMS notification of abnormal errors); in order to solve the problem of memory overflow during task operation, we added an automatic restart function for abnormal exit of subtasks to make your task run more stable, toolkit At the same time support windows, linux, mac environment operation.
</p>
